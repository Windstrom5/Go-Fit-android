...
import com.example.go_fit.adapter.HistoryGymAdapter
import com.example.go_fit.api.MemberApi
import com.example.go_fit.api.historykelasApi
import com.example.go_fit.databinding.ActivityGymHistoryBinding
...
class GymHistoryActivity : AppCompatActivity() , NavigationView.OnNavigationItemSelectedListener{
    ...
    private var adapter: HistoryGymAdapter? = null
    private lateinit var historyAdapter: HistoryGymAdapter
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.menulogout) {
            val builder: AlertDialog.Builder = AlertDialog.Builder(this@GymHistoryActivity)
            builder.setMessage("Want to log out?")
                ...
                .setPositiveButton("YES", object : DialogInterface.OnClickListener {
                    override fun onClick(dialogInterface: DialogInterface, i: Int) {
                        startActivity(Intent(this@GymHistoryActivity, MainActivity::class.java))
                    }
                })
                .show()
            ...
            Response.Listener { response ->
                ...
                val StringRequest2: StringRequest = object : StringRequest(
                    ...
                    Response.Listener { response ->
                        ...
                        val itemList : List<historygym> = gson.fromJson(jsonArray.toString(), Array<historygym>::class.java).toList()
                        recyclerView = binding.rvItem
                        adapter = HistoryGymAdapter(itemList,this)
                        recyclerView.layoutManager = LinearLayoutManager(this)
                        recyclerView.adapter= adapter
                        adapter!!.getVariable(vuser,vpass)
                        recyclerView.visibility = View.VISIBLE
                        if(!itemList.isEmpty()){
                            Toast.makeText(this@GymHistoryActivity,"Data Berhasil Diambil", Toast.LENGTH_SHORT).show()
                        }else{
                            Toast.makeText(this@GymHistoryActivity,"Data Kosong", Toast.LENGTH_SHORT).show()
                        }
                        ...
                        try{
                            ...
                            Toast.makeText(
                                this@GymHistoryActivity,
                                errors.getString("message"),
                                Toast.LENGTH_SHORT
                            ).show()
                        }catch (e: Exception){
                            Toast.makeText(this@GymHistoryActivity,e.message, Toast.LENGTH_SHORT).show()
                        }
                    }
                ){
