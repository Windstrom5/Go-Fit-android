...
import com.example.go_fit.adapter.HistoryInstrukturAdapter
...
class InstrukturHistoryActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener   {
    ...
    private var adapter: HistoryInstrukturAdapter? = null
    private lateinit var historyAdapter: HistoryInstrukturAdapter
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.menulogout) {
            val builder: AlertDialog.Builder = AlertDialog.Builder(this@InstrukturHistoryActivity)
            builder.setMessage("Want to log out?")
                ...
                .setPositiveButton("YES", object : DialogInterface.OnClickListener {
                    override fun onClick(dialogInterface: DialogInterface, i: Int) {
                        startActivity(Intent(this@InstrukturHistoryActivity, MainActivity::class.java))
                    }
                })
                .show()
    ...
    private fun loadMember(nama_member : String) {
        ...
        val StringRequest: StringRequest = object : StringRequest(
            Method.GET,
            InstrukturApi.GET_BY_USERNAME + nama_member,
            Response.Listener { response ->
                ...
                val StringRequest2: StringRequest = object : StringRequest(
                    Method.GET,
                    historyinstrukturApi.GET_BY_USERNAME +nama,
                    Response.Listener { response ->
                        val gson = Gson()
                        setLoading(false)
                        val jsonObject = JSONObject(response)
                        val jsonArray = jsonObject.getJSONArray("data")
                        val itemList : List<historyinstruktur> = gson.fromJson(jsonArray.toString(), Array<historyinstruktur>::class.java).toList()
                        recyclerView = binding.rvItem
                        adapter = HistoryInstrukturAdapter(itemList,this)
                        recyclerView.layoutManager = LinearLayoutManager(this)
                        recyclerView.adapter= adapter
                        adapter!!.getVariable(vuser,vpass)
                        recyclerView.visibility = View.VISIBLE
                        if(!itemList.isEmpty()){
                            Toast.makeText(this@InstrukturHistoryActivity,"Data Berhasil Diambil", Toast.LENGTH_SHORT).show()
                        }else{
                            Toast.makeText(this@InstrukturHistoryActivity,"Data Kosong", Toast.LENGTH_SHORT).show()
                        }
